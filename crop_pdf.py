...
class PDFCropper:
    ...
    def save_file(self):
        try:
            if self.input_pdf_path:
                # Get the directory and file name from the input PDF path
                input_file_name = os.path.basename(self.input_pdf_path)
                input_file_dir = os.path.dirname(self.input_pdf_path)

                # Construct the output file path by appending "cropped" to the input file name
    ...
    def drag_pdf(self, event):
        try:
            self.offset_x += (event.x - self.drag_start_x) / 72
            self.offset_y += (event.y - self.drag_start_y) / 72
